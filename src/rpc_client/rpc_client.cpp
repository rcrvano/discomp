 ...
 * which processed on the rpc side and on the 
 ...
 * Constructor
 */
DiscompRPC::DiscompRPC(QObject *parent)
	: QObject(parent)
{
	discomp_socket = NULL;
};
 ...
 * Connect to server
 */
void DiscompRPC::doConnect( const QString &serverIP, const QString &serverPort )
{
 ...
 */
void DiscompRPC::showHelpUsage() 
{
	std::cout << std::endl;
	std::cout << " RPC with Discomp Server." << std::endl;
	std::cout << std::endl;

	std::cout << " Examples: " <<std::endl;
	std::cout << "     discomp_rpc --config /etc/discomp/rpc.conf --nodeslist --login vano --password 123" << std::endl;
	std::cout << "     discomp_rpc --login vano --password 123 --verbose --connect 192.168.10.192" << std::endl;
	std::cout << "     discomp_rpc --login vano --password 123 --startprocess dsat " << std::endl;
	std::cout << "     discomp_rpc --login vano --password 123 --host 192.168.1.10 --port 5733 --processeslist " << std::endl;
	std::cout << std::endl;
	...
	std::cout << "     --node_install_module  node_id  module_name  - install specified module on the node " << std::endl;
	std::cout << "     --nodes_clean_failed                 - cleand failed modules list for the nodes" << std::endl;
	...
	std::cout << "     --startsingle    dir_path            - start single task in the dir " << std::endl;
	std::cout << std::endl;
 ...
 * \sa showHelpUsage
 */
void DiscompRPC::parseCommandLine(int argc, char *argv[])
{
	...
	for ( int arg_num=1; arg_num<argc; arg_num++ ) {
		...
		if ( argument_str.startsWith("--") ) {        //it's a option name like --node
			...
			//user auth data
			} else if ( argument_str == "--login" || argument_str == "--username" || argument_str == "--user" ) {
			...
			} else if ( argument_str == "--node_install_module" ) {
				method_id = 105;
				method_name = "installModuleOnNode";

				QString argument_value_str (argv[++arg_num]); //get next argument
				if ( argument_value_str.startsWith("-") ) { this->showHelpUsage(); exit(0); };
				QString argument_value_str1 (argv[++arg_num]); //get next argument
				if ( argument_value_str1.startsWith("-") ) { this->showHelpUsage(); exit(0); };

				base_xml_content += "<node id='"+argument_value_str+"'><module name='"+argument_value_str1+"'/></node>\n";

			} else if ( argument_str == "--nodes_clean_failed" ) {
				method_id = 108;
				method_name = "nodesCleanFailed";
			...
			} else if ( argument_str == "--startsingle" ) {
				method_id = 115;
				method_name = "startSingleProcess";

				QString argument_value_str (argv[++arg_num]); //get next argument
				if ( argument_value_str.startsWith("-") ) { this->showHelpUsage(); exit(0); };

				base_xml_content += "<process dir='"+argument_value_str+"'/>\n";
	...
	if ( login == "" ) {
		...
		char input_line[50];
		std::cin >> input_line;
		login = QString(input_line);
	};
	if ( password == "" ) {
		std::cout << "Password: ";
		char input_line[50];
		std::cin >> input_line;
		password = QString(input_line);
	};
	...
	if ( method_id == -1 ) { 
		...
		xml_command_for_send += "<request id='"+QString("%1").arg(method_id)+"' methodName='DiscompRPC."+method_name+"' login='"+login+"' password='"+File::getStrMD5(password)+"'>\n";
		xml_command_for_send += base_xml_content;
		xml_command_for_send += "</request>";
	};
 ...
 */
void DiscompRPC::connectedToServer_slot()
{
 ...
 */
void DiscompRPC::connectionClosed_slot()
{
 ...
 */
void DiscompRPC::socketError_slot ( int code ) {
 ...
 * */
void DiscompRPC::processAnswerFromServer_slot( const QString &xml_data_str )
{
	QString errorStr;
	int errorLine;
	int errorColumn;
	...
	switch ( answerTypeId ) {
		...
		case 103:
			...
			break;
		case 105:
			std::cout << answerNode.toElement().attribute("msg").toStdString() << std::endl;
			break;
		//TODO. where 106, and 107????
		case 108:
			std::cout << answerNode.toElement().attribute("msg").toStdString() << std::endl;
			break;
 ...
 */
void DiscompRPC::showNodesList ( const QDomNode &root_node )
{
	logMessage (LOG_DEBUG,"Processing connected nodes list");
	std::cout << "Connected nodes list:" << std::endl;

	QVector<QString> columns;
	columns << "id" << "ip" << "name" << "ping" << "busy" << "process_id" << "process_name" << "module_name" << "module_list_num" << "module_exec_time" << "ping";
 ...
 */
void DiscompRPC::showNodeInfo ( const QDomNode &root_node )
{
	...
	std::cout << "Modules:\t" << node_info.toElement().attribute("modules_list").toStdString() << std::endl;
	std::cout << "Failed modules:\t" << node_info.toElement().attribute("failed_modules_list").toStdString() << std::endl;
 ...
 */
void DiscompRPC::showProcessesList ( const QDomNode &root_node )
{
 ...
 */
void DiscompRPC::showProcessInfo ( const QDomNode &root_node )
{
	logMessage (LOG_DEBUG,"Processing node info");
 ...
 */
void DiscompRPC::printFormatTable ( QVector<QString> &columns, QVector <QStringList> &rows )
{
 ...
 */
void DiscompRPC::logMessage ( int log_level, const QString &message ) 
{
	Log::addMessage ( log_level, "DiscompRPC", message);
};
