...
globle SLOT_DESC *FindClassSlot(
  ...
  SYMBOL_HN *sname)
  {
   ...
   for (i = 0 ; i < cls->slotCount ; i++)
     {
      if (cls->xslots[i].slotName->name == sname)
        return(&cls->xslots[i]);
     }
   return(NULL);
  }
...
LOCALE void RemoveDefclass(
  ...
  void *vcls)
  {
      ...
      if (cls->xslots[i].defaultValue != NULL)
        {
         if (cls->xslots[i].dynamicDefault)
           ReturnPackedExpression(theEnv,(EXPRESSION *) cls->xslots[i].defaultValue);
         else
           rtn_struct(theEnv,dataObject,cls->xslots[i].defaultValue);
        }
      DeleteSlotName(theEnv,cls->xslots[i].slotName);
      RemoveConstraint(theEnv,cls->xslots[i].constraint);
     }
   ...
   if (cls->slotCount != 0)
     rm(theEnv,(void *) cls->xslots,(sizeof(SLOT_DESC) * cls->slotCount));
      ...
      if (cls->xslots[i].defaultValue != NULL)
        {
         ...
         if (cls->xslots[i].dynamicDefault)
           ...
           rtn_struct(theEnv,dataObject,cls->xslots[i].defaultValue);
#else
         if (cls->xslots[i].dynamicDefault == 0)
           rtn_struct(theEnv,dataObject,cls->xslots[i].defaultValue);
#endif
        }
     }
...
/***************************************************
  ...
  NOTES        : A slot name always has the same
                 ...
                 for immediate lookup of xslots
