...
static void DuplicateMsgHandlerSupport(
  ...
  int msgpass)
  {
   ...
   for (i = 0 ; i < dstins->cls->localInstanceSlotCount ; i++)
     {
      if (dstins->xslots[i].override == FALSE)
        {
         if (msgpass)
           {
            temp.type = (unsigned short)  srcins->xslots[i].type;
            temp.value = srcins->xslots[i].value;
            ...
            DirectMessage(theEnv,dstins->xslots[i].desc->overrideMessage,
                          dstins,&temp,valArg);
         ...
         else
           {
            temp.type = (unsigned short) srcins->xslots[i].type;
            temp.value = srcins->xslots[i].value;
            if (srcins->xslots[i].type == MULTIFIELD)
              {
               SetDOBegin(temp,1);
               SetDOEnd(temp,GetMFLength(srcins->xslots[i].value));
              }
            if (PutSlotValue(theEnv,dstins,&dstins->xslots[i],&temp,&junk,"duplicate-instance")
                 == FALSE)
              goto DuplicateError;
           }
        }
