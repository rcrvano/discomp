...
#pragma unused(buf)
   ...
   for (i = 0 ; i < cls->slotCount ; i++)
     {
      cls->xslots[i].bsaveIndex = ObjectBinaryData(theEnv)->SlotCount++;
      cls->xslots[i].overrideMessage->neededSymbol = TRUE;
      if (cls->xslots[i].defaultValue != NULL)
        {
         if (cls->xslots[i].dynamicDefault)
           {
            ExpressionData(theEnv)->ExpressionCount +=
              ExpressionSize((EXPRESSION *) cls->xslots[i].defaultValue);
            MarkNeededItems(theEnv,(EXPRESSION *) cls->xslots[i].defaultValue);
           }
         else
           {
            /* =================================================
               ...
               ================================================= */
            tmpexp =
              ConvertValueToExpression(theEnv,(DATA_OBJECT *) cls->xslots[i].defaultValue);
   ...
   for (i = 0 ; i < cls->slotCount ; i++)
     {
      if (cls->xslots[i].defaultValue != NULL)
        {
         if (cls->xslots[i].dynamicDefault)
           BsaveExpression(theEnv,(EXPRESSION *) cls->xslots[i].defaultValue,(FILE *) buf);
         else
           {
            /* =================================================
               Static default values are stotred as data objects
...
static void BsaveDefclass(
  ...
  void *buf)
  {
   ...
   if (cls->xslots != NULL)
     {
      dummy_class.xslots = ObjectBinaryData(theEnv)->SlotCount;
      ObjectBinaryData(theEnv)->SlotCount += (long) cls->slotCount;
     }
   else
     dummy_class.xslots = -1L;
...
static void UpdateDefclass(
  ...
  long obji)
  {
   ...
   cls->xslots = SlotPointer(bcls->xslots);
