...
globle int ParseDefclass(
  ...
  char *readSource)
  {
   ...
   while (GetType(DefclassData(theEnv)->ObjectParseToken) != RPAREN)
     {
      ...
      if (strcmp(DOToString(DefclassData(theEnv)->ObjectParseToken),ROLE_RLN) == 0)
        ...
        {
         xslots = ParseSlot(theEnv,readSource,xslots,preclist,FALSE,FALSE);
         if (xslots == NULL)
           {
            parseError = TRUE;
            break;
        ...
        {
         xslots = ParseSlot(theEnv,readSource,xslots,preclist,FALSE,TRUE);
         if (xslots == NULL)
           {
            parseError = TRUE;
            break;
        ...
        {
         xslots = ParseSlot(theEnv,readSource,xslots,preclist,TRUE,TRUE);
         if (xslots == NULL)
           {
            parseError = TRUE;
            break;
   ...
   /* =================================
      Shove xslots into contiguous array
      ================================= */
   if (xslots != NULL)
     PackSlots(theEnv,cls,xslots);
   AddClass(theEnv,cls);
...
static TEMP_SLOT_LINK *MergeSlots(
  ...
  int src)
  {
   ...
   for (i = (int) (cls->slotCount - 1) ; i >= 0 ; i--)
     {
      newSlot = &cls->xslots[i];

      /* ==========================================
         A class can prevent it xslots from being
...
static void PackSlots(
  ...
  TEMP_SLOT_LINK *xslots)
  {
   ...
   cls->xslots = (SLOT_DESC *) gm2(theEnv,(sizeof(SLOT_DESC) * cls->slotCount));
   stmp = xslots;
   for (i = 0 ; i < cls->slotCount ; i++)
     {
      sprv = stmp;
      stmp = stmp->nxt;
      GenCopyMemory(SLOT_DESC,1,&(cls->xslots[i]),sprv->desc);
      cls->xslots[i].sharedValue.desc = &(cls->xslots[i]);
      cls->xslots[i].sharedValue.value = NULL;
