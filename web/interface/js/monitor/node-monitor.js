...
Ext4.onReady(function() {
    ...
    cpuLoadStore = Ext4.create('store.json', { fields: ['m_load', 'm_temp'] });
    ...
    function generateCpuLoad( cpu_value, temp_value ) {
/*
...
*/
		...
		//TODO
            for (var i = 1; i < 100; i++) {
                data.push({
                    m_load: 100,//cpu_value,//generate(data[i - 1].core1),
                    m_temp: 100,//temp_value,//generate(data[i - 1].core2),
                    time: i
                });
            }
		...
		var lastData = cpuLoadStore.last().data;
            cpuLoadStore.loadData([{
                m_load: cpu_value, //generate(lastData.core1),
                m_temp: temp_value, //generate(lastData.core2),
...
//			    generateCpuLoad ( parseInt(stats.cpu.core[0]["@attributes"].used_p), parseInt(stats.cputemp["@attributes"].core0) );
			    generateCpuLoad ( parseInt(stats.cpu.core[0]["@attributes"].used_p), parseInt(stats.cpu.core[1]["@attributes"].used_p) );
                            
			    var edit_mode=response.responseText;
                            if(edit_mode==0){
                                return 1;
