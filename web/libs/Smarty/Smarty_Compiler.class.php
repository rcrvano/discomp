...
/* $Id: Smarty_Compiler.class.php,v 1.6 2008/05/19 08:59:29 vano Exp $ */
...
class Smarty_Compiler extends Smarty {
	...
	/* Telinweb bug with <select> element
	   in the popups if selector too long and select some option
	   then selected somthing wrong (only for Gecko) */
	if ( !$this->disable_select_replace ) {
		$source_content = preg_replace("/<option /Umsi","<option onMouseDown=formSelectOption(this) ",$source_content);
	};

	/* TelinWeb replaces for iPhone */
	/* iPhone don't have dblclick event */
	if ( $GLOBALS['THEME'] == 'iphone' ) {
		$source_content = preg_replace("/onDblClick=/Umsi","onMouseUp=",$source_content);
	};

	/* TelinWeb. for clear images cache after system upgrade.*/
	$source_content = preg_replace("/src=(.*\.jpg)/Ui",'src=$1?version='.APP_VERSION, $source_content);
	$source_content = preg_replace("/src=(.*\.gif)/Ui",'src=$1?version='.APP_VERSION, $source_content);
	$source_content = preg_replace("/src=(.*\.png)/Ui",'src=$1?version='.APP_VERSION, $source_content);

	/* Telinweb. deny forms onsubmit */
	$source_content = preg_replace("/<form>/Umsi", "<form onsubmit='return false'>", $source_content);
