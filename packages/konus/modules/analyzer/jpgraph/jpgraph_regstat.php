...
// Ver:		$Id: jpgraph_regstat.php,v 1.1 2006/03/22 13:12:30 vano Exp $
//
...
class Spline {
	...
	if( $this->n !== count($xdata) ) {
	    JpGraphError::Raise('Spline: Number of X and Y coordinates must be the same');
	}
 ...
 */
	...
	if( $this->n !== count($datay) ) {
	    JpGraphError::Raise('Bezier: Number of X and Y coordinates must be the same');
	}
	$idx=0;
